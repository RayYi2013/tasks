...
var createNode = function (path, properties) {
    ...
    properties["jcr:primaryType"] = "sling:Folder";
...
var getNode = function (path) {
    var _this = exports;
    var defer = q.defer();
    var opt = getDefaultOptions(path);
...
/**
 * Create a node in the Sling instance.
 * the query use url like: http://centos65-ray.cloudapp.net:8080/content.query.json?queryType=xpath&statement=//data/yisc_sohu_com//*
...
var getDescendantNodeList = function (path) {
    var _this = exports;
    var defer = q.defer();
    var opt = getDefaultOptions(encodeURI(util.format('/content.query.json?queryType=xpath&statement=/%s//*',path)));
